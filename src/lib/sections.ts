...
export const SECTIONS: Section[] = [
	{
		...
		icon: 'adjustments',
		group: 'herramientas',
		card: 'JSON a YAML, cron en cristiano, regex, Base64, HTML a PHP, colores, gradientes, sombras, transiciones, animaciones SVG, parallax, maquetador, análisis de imagen, superposición de capturas, tramas, flechas CSS, gráficos, barras, dispersión, diagramas, ASCII art, tema y config.',
		intro:
			'Lo que salió de ideas ganadoras: JSON a YAML, cron en cristiano, probador de regex, Base64 en los dos sentidos, HTML a PHP, colores, gradientes —lineales o radiales, con su ángulo, para copiar el CSS o bajar en PNG—, sombras —box-shadow de varias capas, también por dentro, sobre un cuadro de prueba—, transiciones —qué cambia al pasar el ratón, cuánto dura y con qué curva cubic-bezier, que se edita arrastrando sus puntos—, animaciones SVG —una figura que se mueve, gira, crece o cambia de color, con su duración y su curva, para bajar el .svg animado—, parallax —capas de formas o imágenes tuyas, cada una a su velocidad, probadas con scroll en marcos de móvil, tablet y escritorio, con el HTML, el CSS y el JS para copiar—, maquetador —cajas, textos e imágenes que se arrastran y se estiran sobre una cuadrícula, con plantillas, márgenes y colores, para bajar como .html con su CSS—, análisis de imagen —histograma, paleta dominante, brillo y contraste, sin que la imagen salga de tu navegador—, superposición —dos capturas de una web, una encima de otra, con opacidad, cortinilla o modo diferencia para ver qué cambia—, tramas SVG —quietas o animadas—, flechas CSS —un fondo de flechas con su forma, dirección, tamaño y color, para copiar el CSS—,gráficos de datos —de una tabla o un CSV a SVG y PNG—, diagramas de flujo —nodos que se arrastran y se unen con flechas, guardados en tu navegador—, ASCII art —tu texto en letras grandes, en varios estilos, para copiar o bajar como .txt—, tema —claro u oscuro, con plantillas de color— y config. Cada pestaña tiene su propio enlace para compartirla.',
	},
	{
		href: '/playground',
		title: 'Playground',
